...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getSubCategoryById() {
    const data = await prisma.sub_category.findFirst(convertQueryToPrismaUtil(req.query, 'sub_category'));
    return res.status(200).json(data);
  }

  async function updateSubCategoryById() {
    await subCategoryValidationSchema.validate(req.body);
    const data = await prisma.sub_category.update({
  ...
  async function deleteSubCategoryById() {
    const data = await prisma.sub_category.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
